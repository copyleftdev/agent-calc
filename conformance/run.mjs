...
for (let i = 0; i < corpus.length; i += 1) {
  ...
  for (const [what, got] of pairs) {
    if (!same(base, got)) {
      failed += 1;
      console.log(`\n  DIVERGED  ${c.id}  (${c.command})  cli vs ${what}`);
      const a = JSON.stringify(canon(base));
      const b = JSON.stringify(canon(got));
      let k = 0;
      while (k < a.length && a[k] === b[k]) k += 1;
      console.log(`    cli   …${a.slice(Math.max(0, k - 40), k + 60)}`);
      console.log(`    ${what.padEnd(5)} …${b.slice(Math.max(0, k - 40), k + 60)}`);
    }
  }
}
